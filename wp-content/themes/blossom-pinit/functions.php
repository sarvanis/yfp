...
function blossom_pinit_styles() {
	...
	wp_enqueue_style( 'blossom-pin', get_template_directory_uri() . '/style.css', array( 'owl-carousel' ) );

	wp_enqueue_style( 'blossom-pinit', get_stylesheet_directory_uri() . '/style.css', array( 'blossom-pin' ), $version );
...
function blossom_pinit_customizer_register( $wp_customize ) {
    ...
    $theme_info .= '</p><p>';
    $theme_info .= sprintf( __( 'Documentation Link: %1$sClick here.%2$s', 'blossom-pinit' ),  '<a href="' . esc_url( 'https://docs.blossomthemes.com/docs/blossom-pinit-free-theme-documentation/' ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p>';
...
function blossom_pin_dynamic_css(){
    ...
    .banner-slider .item .text-holder .entry-title a{
        background-image: linear-gradient(180deg, transparent 95%,  <?php echo blossom_pin_sanitize_hex_color( $primary_color ) ?> 0);
    }
    ...
    <div class="footer-b">
        <div class="container">
            ...
            <?php
                ...
                echo '<a href="' . esc_url( 'https://blossomthemes.com/downloads/blossom-pinit-free-wordpress-theme/' ) .'" rel="nofollow" target="_blank">'. esc_html__( 'Blossom Themes', 'blossom-pinit' ) . '</a>.';
